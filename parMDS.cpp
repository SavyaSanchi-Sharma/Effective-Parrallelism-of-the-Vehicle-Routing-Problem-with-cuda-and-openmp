...
std::vector<double> gpuEvaluateBatch(const VRP& vrp,const std::vector<std::vector<int>>& batchPerms);
...
postProcessIt(const VRP &vrp, std::vector<std::vector<node_t>> &final_routes, weight_t &minCost) {
  ...
  // Try TSP approximation + 2-OPT path
  ...
  // Try direct 2-OPT path
  auto postprocessed_final_routes3 = postprocess_2OPT(vrp, final_routes);

  // Calculate costs once for each complete solution
  weight_t cost2 = get_total_cost_of_routes(vrp, postprocessed_final_routes2);
  weight_t cost3 = get_total_cost_of_routes(vrp, postprocessed_final_routes3);

  // Select the better solution
  if (cost2 < cost3) {
    minCost = cost2;
    return postprocessed_final_routes2;
  } else {
    minCost = cost3;
    return postprocessed_final_routes3;
  }
}
...
int main(int argc, char *argv[]) {
  ...
  const int TOTAL_ITERS = 100000;
  const int BATCH_SIZE  = 2046; 
initGPU(vrp, BATCH_SIZE);
  static std::mt19937 rng(std::random_device{}());

  for (int iter = 0; iter < TOTAL_ITERS; iter += BATCH_SIZE) {

      std::vector<std::vector<int>> batchPerms;
batchPerms.resize(BATCH_SIZE);

#pragma omp parallel
{
    std::mt19937 local_rng(omp_get_thread_num() + 1234);

    #pragma omp for
    for (int b = 0; b < BATCH_SIZE; b++) {

        auto localMST = mstCopy;   // thread-local copy

        for (auto &list : localMST)
            std::shuffle(list.begin(), list.end(), local_rng);

        std::vector<int> singleRoute;
        std::vector<bool> visited(localMST.size(), false);
        visited[0] = true;

        ShortCircutTour(localMST, visited, 0, singleRoute);

        batchPerms[b] = singleRoute;
    }
}
      auto costs = gpuEvaluateBatch(vrp, batchPerms);

      for (int b = 0; b < BATCH_SIZE; b++) {
          if (costs[b] < minCost) {
              minCost  = costs[b];
              minRoute = convertToVrpRoutes(vrp, batchPerms[b]);
          }
      }
  }
